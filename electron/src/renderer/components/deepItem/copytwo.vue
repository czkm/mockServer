<template>
  <div class="deepitems" v-if="data" :style="`${ml ? '' : 'margin-left:55px'}`">
    ...
    <!-- <Zfitem label='88' v-for="(item,index) in 5" v-model="index"></Zfitem> -->
  </div>
</template>

<script>
import Zfitem from '../zfitem/index'
...
export default {
  ...
  methods: {
    setkey(item){
      console.log('setkey',item)
      console.log('data',this.ipt);
      this.$set(this.ipt,item.nw,this.ipt[item.old])
    },
    isipt(item) {
    ...
    addone() {
      if (this.isaddarr) {
        ...
        if (this.isarr == 'Object' || this.isarr == 'Array') {
          ...
          this.$set(this.ipt[this.addindex], this.vmkey, this.vmvalue)
          // this.ipt[this.addindex][this.vmkey] = this.vmvalue
        }
      }
      // console.log('type', this.isaddarr, this.isarr)
    ...
    addshow(index) {
      ...
      // console.log('this', this.ipt[this.addindex] instanceof Array);
    },
    isshowbut() {
      if (this.isaddarr) {
        if (this.isarr == '其它类型') {
